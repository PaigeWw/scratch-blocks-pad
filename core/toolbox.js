...
Blockly.Toolbox.prototype.init = function() {
  ...
  Blockly.bindEventWithChecks_(this.HtmlDiv, 'click', this,
      function(e) {
  ...
  this.trash = goog.dom.createDom('div', 'mxcTrash');
  this.HtmlDiv.appendChild(this.trash);
  this.position();
};
...
Blockly.Toolbox.prototype.setSelectedItem = function(item) {
  if (this.selectedItem_) {
    ...
    this.selectedItem_.bubble_.style.backgroundImage = this.selectedItem_.bg_;
  }
...
Blockly.Toolbox.CategoryMenu.prototype.createDom = function() {
  ...
  */
  this.ul = goog.dom.createDom('ul', 'scratchCategoryMenu');
  this.parentHtml_.appendChild(this.ul);
};
...
Blockly.Toolbox.CategoryMenu.prototype.populate = function(domTree) {
  ...
  var columnSeparator = Math.ceil(categories.length);
  for (var i = 0; i < columnSeparator; i += 1) {
    child = categories[i];
    var row = goog.dom.createDom('li', 'scratchCategoryMenuRow');
  ...
  this.height_ = this.ul.offsetHeight;
};
...
Blockly.Toolbox.Category = function(parent, parentHtml, domTree) {
  ...
  this.bg_ = domTree.getAttribute('bg');
  this.bgSelected_ = domTree.getAttribute('bgSelected');
